...
- **Exa Search API** (via manage_websets) — `exa.search` (instant web search with category/date/domain filters), `exa.findSimilar` (find pages similar to a URL), `exa.getContents` (extract text/highlights/summary from URLs), `exa.answer` (question answering with citations)
- **Exa Task Workflows** (via manage_websets tasks.create) — `retrieval.searchAndRead` (search + read pages), `retrieval.expandAndCollect` (search + findSimilar expansion + dedup), `retrieval.verifiedAnswer` (answer + independent validation)
- **Exa Webset Workflows** (via manage_websets tasks.create) — `lifecycle.harvest`, `convergent.search`, `adversarial.verify`, `qd.winnow`, `research.deep`, `research.verifiedCollection`
