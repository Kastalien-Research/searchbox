...
After completing a taste evaluation, log for quality tracking. The existing `taste_evaluations` table already captures most data. Additionally, run these self-checks:

- [ ] Verdict is one of: proceed, simplify, defer, kill
- [ ] Compression is a single sentence (no line breaks, under 200 chars)
- [ ] Rationale logically supports the verdict (not just restating it)
- [ ] Only sections that were evaluated are included (per meta-pruning)

When a previous taste evaluation has `followed_up = 1` and `outcome_notes`, compare the original verdict against the actual outcome. This is the most valuable calibration signal — track whether your verdicts are predictive.
