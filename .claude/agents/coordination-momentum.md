...
7. **Quality signals**: Recent eval results if available (query `agent_quality_summary` view)

## Post-Session Evaluation

After generating a status report, self-check:

- [ ] All 6 required sections present (active, blocked, available, conflicts, spirals, recommendation)
- [ ] No recommendations that change priority levels (reordering within level is OK)
- [ ] Reported issue states match actual `bd list` output
- [ ] If all workstreams blocked, escalation recommended
