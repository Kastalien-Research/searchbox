...
After completing a fix (Phase 4), log an evaluation record for quality tracking:

```bash
# Log the fix outcome for evaluation
sqlite3 research-workflows/workflows.db "INSERT INTO eval_results (experiment_id, example_id, grader_id, agent_output, score, passed, comment) VALUES ('online-triage', '<beads_issue_id>', 'triage_format_compliance', '<json_output>', <score>, <passed>, '<comment>');"
```

Self-check before closing:
- [ ] Output includes all 6 required sections (root cause, classification, fix, before, after, regression)
- [ ] Files modified are within the original failure scope
- [ ] If external dependency issue, escalation was sent (not a fix attempt)
